...
public class CacheEvictionTest extends TestCase {
  ...
  public void testEviction_lru() {
    ...
    CacheTesting.drainRecencyQueues(cache);
    assertThat(keySet).has().allOf(6, 7, 8, 9, 0, 1, 2, 10, 11, 12);

    // re-order
    getAll(cache, asList(6, 7, 8));
    CacheTesting.drainRecencyQueues(cache);
    assertThat(keySet).has().allOf(9, 0, 1, 2, 10, 11, 12, 6, 7, 8);

    // evict 9, 0, 1
    getAll(cache, asList(13, 14, 15));
    CacheTesting.drainRecencyQueues(cache);
    assertThat(keySet).has().allOf(2, 10, 11, 12, 6, 7, 8, 13, 14, 15);
  }
  ...
  public void testEviction_weightedLru() {
    ...
    CacheTesting.drainRecencyQueues(cache);
    assertThat(keySet).has().allOf(6, 7, 8, 9, 0, 1, 2, 10);

    // re-order
    getAll(cache, asList(6, 7, 8));
    CacheTesting.drainRecencyQueues(cache);
    assertThat(keySet).has().allOf(9, 0, 1, 2, 10, 6, 7, 8);

    // evict 9, 1, 2, 10
    getAll(cache, asList(15));
    CacheTesting.drainRecencyQueues(cache);
    assertThat(keySet).has().allOf(0, 6, 7, 8, 15);

    // fill empty space
    getAll(cache, asList(9));
    CacheTesting.drainRecencyQueues(cache);
    assertThat(keySet).has().allOf(0, 6, 7, 8, 15, 9);

    // evict 6
    getAll(cache, asList(1));
    CacheTesting.drainRecencyQueues(cache);
    assertThat(keySet).has().allOf(0, 7, 8, 15, 9, 1);
  }
  ...
  public void testEviction_overweight() {
    ...
    CacheTesting.drainRecencyQueues(cache);
    assertThat(keySet).has().allOf(0, 45);
  ...
  public void testEviction_invalidateAll() {
    ...
    CacheTesting.drainRecencyQueues(cache);
    assertThat(keySet).has().allOf(0, 1, 2, 3, 4);
    ...
    CacheTesting.drainRecencyQueues(cache);
    assertThat(keySet).has().allOf(5, 6, 7, 8, 9, 10, 11, 12);
  }
